...
本脚本负责从网络上获取福彩3D的历史开奖数据，并将其与本地的CSV文件合并，
最终生成一个全面、更新的数据文件。

主要功能:
1.  从文本文件 (3d_asc.txt) 获取包含开奖日期的完整历史数据。
2.  从HTML网页抓取最新的开奖数据（可能不含日期），作为补充。
3.  将两种来源的数据智能合并到主CSV文件 ('fc3d.csv') 中。
    - 优先使用文本文件中的数据（尤其是日期）。
    - 能够处理新旧数据，自动去重和更新。
...
TXT_DATA_URL = 'https://data.17500.cn/3d_asc.txt'
# HTML源：提供最新的开奖数据，通常用于快速更新（但不含日期）
HTML_DATA_URL = "https://www.17500.cn/chart/3d-tjb.html"
...
logger = logging.getLogger('fc3d_data_processor')
...
def fetch_latest_data_from_html(url: str = HTML_DATA_URL) -> list:
    ...
    Returns:
        list: 一个包含字典的列表，每个字典代表一期数据，格式为
              ...
              如果失败则返回空列表。
    """
    logger.info("正在从HTML网页抓取最新福彩3D数据...")
    data = []
    try:
        ...
        for row in table.find_all('tr')[1:]:  # 跳过表头
            ...
            try:
                ...
                # 验证号码格式并补齐为3位数字
                if not number_text.isdigit() or len(number_text) > 3:
                    continue
...
def parse_txt_data(data_lines: list) -> list:
    ...
    for line in data_lines:
        ...
        if len(fields) < 3:
            continue  # 忽略格式不正确的行
        try:
            # 数据格式: [期号, 日期, 号码] 或 [期号, 日期, 数字1, 数字2, 数字3]
            qihao = fields[0]
            date = fields[1]
            
            # 处理3D号码，可能是整体一个字段或分开的三个字段
            if len(fields) == 3:
                # 格式: 期号 日期 号码(如123或73)
                number = fields[2]
            elif len(fields) >= 5:
                # 格式: 期号 日期 数字1 数字2 数字3
                number = ''.join(fields[2:5])
            else:
                continue
            
            # 将号码补齐为3位数字（前面补0）
            if number.isdigit():
                number = number.zfill(3)  # 补齐为3位，例如 73 -> 073
                
            # 基础验证
            if qihao.isdigit() and len(date.split('-')) == 3 and len(number) == 3 and number.isdigit():
                parsed_data.append([qihao, date, number])
        except (IndexError, ValueError):
            logger.warning(f"解析TXT行时出错: {line}")
            continue
...
if __name__ == "__main__":
    ...
    txt_data_dicts = [{'期号': r[0], '日期': r[1], '号码': r[2]} for r in txt_parsed_list]
